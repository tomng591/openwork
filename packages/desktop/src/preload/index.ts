...
export interface GeminiAPI {
  ...
  onToolCallsUpdated: (callback: (toolCalls: unknown[]) => void) => () => void;
}
...
contextBridge.exposeInMainWorld('api', {
  gemini: {
    ...
    send: (message, promptId) =>
      ipcRenderer.invoke('gemini:send', message, promptId),
    ...
    onStreamEvent: (callback) =>
      createEventHandler('gemini:stream-event', callback),
    ...
    onToolCallsUpdated: (callback) =>
      createEventHandler('tool:calls-updated', callback),
  },

  tools: {
    approve: (callId) => ipcRenderer.invoke('tools:approve', callId),
    reject: (callId, reason) =>
      ipcRenderer.invoke('tools:reject', callId, reason),
  },

  notifications: {
    show: (title, body) => ipcRenderer.send('notifications:show', title, body),
    showWithActions: (options) =>
      ipcRenderer.invoke('notifications:show-with-actions', options),
  },

  menu: {
    onPreferences: (callback) =>
      createEventHandler('menu:preferences', callback),
    onNewConversation: (callback) =>
      createEventHandler('menu:new-conversation', callback),
    onOpenProject: (callback) =>
      createEventHandler('menu:open-project', callback),
    onExportConversation: (callback) =>
      createEventHandler('menu:export-conversation', callback),
    onClearConversation: (callback) =>
      createEventHandler('menu:clear-conversation', callback),
    onToggleFileViewer: (callback) =>
      createEventHandler('menu:toggle-file-viewer', callback),
    onStopGeneration: (callback) =>
      createEventHandler('menu:stop-generation', callback),
    onApproveAll: (callback) =>
      createEventHandler('menu:approve-all', callback),
